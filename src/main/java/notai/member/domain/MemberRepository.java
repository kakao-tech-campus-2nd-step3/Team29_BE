...
public interface MemberRepository extends JpaRepository<Member, Long> {
    ...
    default Member getById(Long id) {
        return findById(id).orElseThrow(() -> new NotFoundException("회원 정보를 찾을 수 없습니다."));
    }

    Optional<Member> findByRefreshToken(String refreshToken);

    default Member getByRefreshToken(String refreshToken) {
        return findByRefreshToken(refreshToken).orElseThrow(() -> new UnAuthorizedException("유효하지 않은 Refresh Token입니다."));
    }
}
